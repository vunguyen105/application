			...
			<div id="content" class="portlet-body flip-scroll">
				<table id="table_product"
					...
					<thead class="flip-content">
					 ...
					 <?php $i = 1; foreach ( $slides as $key => $value ) :?>
					<tbody>
						<tr>
							<td><?php echo $i; ?></td>
                            ...
                            <td data-id="<?php echo $value['SlideID']?>"><span
								class="icon-edit"></span></td>
							<td data-no="<?php echo $i;?>"
								data-id="<?php echo $value['SlideID']?>"><span class="icon-trash"></span></td>
						</tr>
					</tbody>
                       <?php $i++; endforeach; ?>
				</table>
...
jQuery(document).on('click', 'table#table_product span.icon-trash', function() {
    ...
    jQuery(document).on('click', 'span.icon-edit', function() {
    	var id = $(this).parent().attr('data-id');
    	window.open(
    	  '<?php echo base_url('slide/edit');?>'+'/'+id,
    	  '_blank' 
    	);
